...
class ImageSequence(keras.utils.Sequence):
    ...
    def __getitem__(self, idx):
        ...
        for i in range(self.batch_size):
            ...
            img = Image.open(os.path.join(self.directory_name, random_image_file))
            img = ImageOps.autocontrast(img.convert(mode="L"), cutoff=10)
            img_data = numpy.expand_dims( numpy.array(img) ,2)
            processed_data = numpy.array(img_data) / 255.0
            X[i] = processed_data
...
def main():
    ...
    # with tf.device('/device:XLA_CPU:0'):
        ...
        callbacks = [keras.callbacks.EarlyStopping(monitor='val_loss', patience=2, verbose=True, restore_best_weights=True),
